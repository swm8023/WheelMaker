...
describe('web chat integration', () => {
  test('defines registry session protocol and uses real chat UI instead of placeholder sessions', () => {
    ...
    expect(mainTsx).toContain('mergeIncomingSessionMessage,');
    expect(mainTsx).toContain('const incomingMerge = mergeIncomingSessionMessage(');
    expect(mainTsx).toContain('const merged = incomingMerge.messages;');
    expect(mainTsx).toContain('const latestSyncCursor = incomingMerge.cursor;');
    expect(mainTsx).toContain('if (incomingMerge.gapReadCursor) {');
    ...
    const incomingMerge = mainTsx.indexOf('const incomingMerge = mergeIncomingSessionMessage(', promptDoneSignal);
    const incomingStoreApply = mainTsx.indexOf('chatMessageStoreRef.current[runtimeKey] = merged;', incomingMerge);
    const incomingVisibleApply = mainTsx.indexOf('setVisibleChatMessagesForRuntimeKey(runtimeKey, merged, {', incomingMerge);
    const promptGapRead = mainTsx.indexOf('if (incomingMerge.gapReadCursor) {', incomingMerge);
    expect(promptDoneSignal).toBeGreaterThanOrEqual(0);
    expect(promptDoneStateApply).toBeGreaterThan(promptDoneSignal);
    expect(incomingMerge).toBeGreaterThan(promptDoneStateApply);
    expect(incomingStoreApply).toBeGreaterThan(incomingMerge);
    ...
    const eventMerge = mainTsx.indexOf('const incomingMerge = mergeIncomingSessionMessage(', eventRunningSignal);
    const eventGapRead = mainTsx.indexOf('if (incomingMerge.gapReadCursor) {', eventMerge);
    expect(eventRunningSignal).toBeGreaterThanOrEqual(0);
    expect(eventRunningApply).toBeGreaterThan(eventRunningSignal);
    expect(eventMerge).toBeGreaterThan(eventRunningApply);
    expect(eventGapRead).toBeGreaterThan(eventMerge);
