...
describe('web file pin feature', () => {
  test('supports pinning files and switching via pinned list', () => {
    ...
    expect(mainTsx).toContain("className={`file-action-panel side-action-panel ${gotoToolsOpen ? 'open' : ''}`}");
    expect(mainTsx).toContain("className={`file-action-panel side-action-panel ${searchToolsOpen ? 'open' : ''}`}");
