...
## 跨平台开发策略（Web / Desktop / iOS / Android）

- Flutter 支持同时覆盖 `web`、`desktop`、`ios`、`android`，本项目按跨平台目标推进。
- 开发阶段默认优先适配浏览器（Web）：热重载快、调试链路短、迭代成本最低。
- 采用“Web 先行 + 移动端持续验证”策略：
  - 日常功能开发与 UI 迭代先在 Web 完成。
  - 至少每周执行一次 `flutter run`（Android/iOS）与关键页面冒烟，提前发现平台差异。
- 依赖选型优先级：优先选择同时支持 `web + desktop + mobile` 的插件，避免后期替换。
- 任何涉及平台能力（文件系统、通知、相机、后台能力）必须在设计阶段标注平台差异与降级方案。
