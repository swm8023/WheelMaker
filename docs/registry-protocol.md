...
## 0. WheelMaker Global Protocol Envelope
...
1. **协议版本硬切换**：`connect.init.payload.protocolVersion` 必须为 `2.3`，不接受 `2.2` Hub / Web 连接。

2. **Codex agent identity 改名**：外部协议中的 `codex` 现在表示 `codex app-server` provider；旧 ACP adapter 改名为 `codexacp`，仅用于已托管历史 session，不再出现在新建或 resume/import 入口。

3. **NPM runtime 映射更新**：`@openai/codex` 绑定 `agentTypes=["codex"]`，`@zed-industries/codex-acp` 绑定 `agentTypes=["codexacp"]`。
