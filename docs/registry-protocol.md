...
| `client` | `project.list`、`project.syncCheck`、`chat.send`、`chat.permission.respond`、`session.*`、`fs.*`、`git.*`、`batch` |
...
客户端按 `staleDomains` 进行增量刷新。当前实现以 `project`、`git`、`worktree` 为主；当仅 `projectRev` 漂移且未命中前述域时，服务端会返回 `fs` 作为兜底域。
...
- Chat: `chat.send`、`chat.permission.respond`
...
2. 按 `staleDomains` 分流：
   - 包含 `project` 或 `fs`：刷新 project 元信息；按可见范围执行 `fs.list` / `fs.read`。
