...
- [Registry Protocol 2.3 specification](#1-目标与范围) — 本文第 1 章起
...
`relayUrl` 是 Registry 根据 control plane 连接的 Host / `X-Forwarded-Host` 和 TLS / `X-Forwarded-Proto` 推导出的 data-plane 访问地址。App 可以在代理头缺失导致 `relayUrl` 为 loopback、但当前 registry address 是非 loopback 地址时，用当前 registry address 和 `listenPort` 重新推导打开地址；本地 EXE 连接本机 Registry 时应继续使用 loopback URL。
