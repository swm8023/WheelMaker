...
- ä¸å¯ä»¥ï¼šä»»ä½•æœªå°† `internal/agent/provider/` ä½œä¸ºç‹¬ç«‹åŒ…å¹¶å¼•å…¥ `Adapter` æŽ¥å£çš„æž¶æž„ã€‚
...
Adapter å±‚ + Clientï¼ˆä¾èµ–ä»¥ä¸Šä¸¤éƒ¨åˆ†å®Œæˆï¼‰ï¼š
  - internal/agent/provider/provider.go:       Adapter æŽ¥å£
  - internal/agent/provider/codex/provider.go: CodexAdapterï¼ˆResolveBinary + Conn.Startï¼‰
...
3. **Adapter å±‚ + Client é‡æž„**ï¼šåˆ›å»º Adapter æŠ½è±¡ï¼Œä»¥ Client æ›¿æ¢ Hubã€‚
   - æ­¥éª¤ Aï¼šåˆ›å»º `internal/agent/provider/provider.go`ï¼ŒåŒ…å« `Adapter` æŽ¥å£ã€‚
   - æ­¥éª¤ Bï¼šåˆ›å»º `internal/agent/provider/codex/provider.go`ï¼ŒåŒ…å« `CodexAdapter`ï¼ˆä½¿ç”¨ `tools.ResolveBinary`ï¼Œåˆ›å»º `acp.Conn`ï¼Œè°ƒç”¨ `Conn.Start()`ï¼Œè¿”å›žå·²å¯åŠ¨çš„ `Conn`ï¼‰ã€‚
...
```go
// agent/acp/connect.go â€” moved into agent/acp/, renamed, logic unchanged
package acp
