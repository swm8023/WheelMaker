...
- `internal/adapter/` (adapter interface + codex adapter)
...
- Per project:
  ...
  - Calls `c.RegisterAdapter("codex", codex.NewAdapter(...))`
...
// ConsoleIM implements im.Adapter.
...
## 4. internal/agent/acp/conn.go — debug logging
...
The debug writer is injected by `client.Client` after `adapter.Connect()` returns the `*acp.Conn`, using the debug flag from the console IM config.
...
**Remove** the eager `adapter.Connect()` call from `Start()`.
...
ensureAgent:
  ...
  fac = c.adapterFacs[c.state.ActiveAdapter]
...
main.go
  ...
  → hub.Start(ctx)
      ...
      → per project: c.RegisterAdapter("codex", codex.NewAdapter(...))
...
2. `internal/agent/acp/conn.go` — add SetDebugLogger()
