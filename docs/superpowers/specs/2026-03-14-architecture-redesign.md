...
// 子进程的生命周期由返回的 *acp.Conn 持有；Adapter.Close() 仅用于 Connect() 失败时的清理。
// Connect() 成功后调用 Adapter.Close() 是 no-op（子进程已由 Conn 管理）。
...
```go
// agent/acp/conn.go — 移入 agent/acp/，并重命名，逻辑不变
package acp
...
有 IM adapter 时调用 `im.Adapter.Run(ctx)`。
...
| `adapter/codex` | 集成测试，`//go:build integration`，需真实 codex-acp binary |
