...
- iframe URL 使用当前 registry address 推导 relay URL：
  ...
  - 如果 `relay.status` / `relay.enable` 返回的 `relayUrl` 是 `127.0.0.1` / `localhost` / `::1`，但当前 registry address 是非 loopback 地址，普通浏览器 App 使用当前 registry address 重新推导 URL。这主要兜底代理未透传 `Host` / `X-Forwarded-*` 时 Registry 返回 loopback URL 的情况。
  - Windows EXE desktop bridge / remote web source 模式下，App 认为 snapshot `relayUrl` 是可直连地址并优先使用，不把 loopback relay URL 改写成外网 registry address。
