...
- 从 `relay.status` 初始化。
- 打开页面时如果本地没有 6 位访问码，前端自动生成；重新生成时调用 `relay.regenerateAccessCode`。
- 设置页不显示可编辑 target host。启用时固定发送 `targetHost="127.0.0.1"`。
- `Hub Local Port` 默认 `80`。
...
- Settings detail 包含 Port Relay 页面，默认 Hub Local Port 为 80，不显示 Target Host 输入。
- 6 位访问码生成。
...
2. 用户可以选择在线 Hub、设置 listen port、Hub local port，并生成 6 位访问码。
