...
App 内打开 relay iframe 时，如果本机知道当前 `accessCodeGeneration` 对应的 6 位访问码，可以给 iframe URL 附加内部查询参数 `__wm_relay_code=<code>`。Registry data plane 在普通路径认证前先消费该参数：

- code 正确：设置 `wm_port_relay` HttpOnly cookie，并 303 Redirect 到移除 `__wm_relay_code` 后的原同站相对路径。
- code 错误：303 Redirect 到登录页，`next` 使用移除 `__wm_relay_code` 后的路径。
- `__wm_relay_code` 只允许 Registry 消费，不能进入 Hub tunnel，也不能转发给第三方目标服务。
- 如果 App 不知道当前明文访问码，不能附加该参数，仍显示设置页要求用户 Generate/Reset。
...
- 移动端 relay 悬浮按钮在 iframe 已打开时支持长按切换 target：长按在按钮旁显示小菜单，包含所有本地 preset，当前 active target 置顶并打勾，其余按保存顺序展示，文案为 `hubId:port`。点击当前项只关闭菜单；点击其它项立即调用现有 `relay.enable` 替换当前单例 slot，并把 iframe 路径重置为 `/`。如果本机不知道当前 6 位访问码，菜单关闭并打开 Port Relay 设置页，要求用户先 Generate/Reset。iframe 未打开时长按不做任何操作。
- Chat 消息中的 `http://localhost:<port>` 和 `http://127.0.0.1:<port>` 链接作为本地服务入口处理。支持 Markdown 链接、GFM 自动链接，以及 inline code 中只有本地 HTTP URL 的形态（例如 `` `http://localhost:58647` ``）。用户点击后，App 使用当前选中项目的 `hubId` 自动新增/选中 `{hubId, targetPort}` preset，启用或切换 relay，并把右侧/移动端 iframe 打开到原链接的 path、query、hash。普通外部链接和 `https://localhost` 不走 relay。
...
- iframe URL 使用当前 registry address 推导 relay URL：
  ...
  - 如果 `relay.status` / `relay.enable` 返回的 `relayUrl` 是 `127.0.0.1` / `localhost` / `::1`，但当前 registry address 是非 loopback 地址，App 使用当前 registry address 重新推导 URL。这主要兜底代理未透传 `Host` / `X-Forwarded-*` 时 Registry 返回 loopback URL 的情况；Windows EXE remote web bridge 模式也遵循这个规则。
  - 只有当前 registry address 本身是 loopback 时，App 才保留 loopback `relayUrl`。
  - App 内 iframe 打开时，如果本机知道当前访问码，URL 追加 `__wm_relay_code=<code>` 触发 Registry 自动认证；Registry 必须消费并重定向到不含该参数的 URL，不能把参数转发给第三方页面。
