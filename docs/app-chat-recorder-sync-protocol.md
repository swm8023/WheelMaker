...
> **Frontend processing**: Messages from `session.read` are decoded by `decodeSessionMessageFromEventPayload`, then grouped by `syncIndex` (= `promptIndex`) into a `prompts` array. The final return is `{session, prompts, messages}` for UI consumption.
...
4. Dedup via `(syncIndex, syncSubIndex)` cursors (only process newer messages).
