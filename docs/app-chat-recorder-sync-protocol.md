...
> **`done` is turn envelope state**: only `agent_message_chunk` / `agent_thought_chunk` use it, and it means that turn is sealed; it is not a prompt completion marker. `content.param.text` is always the full text snapshot for the current turn.
...
4. Dedup via `(promptIndex, turnIndex)` cursors; `prompt_done` does not advance the read cursor.
...
7. On `prompt_done`, if the current prompt has an undone text/thought turn, or local max turn is less than `prompt_done.turnIndex - 1`, call `session.read(sessionId, promptIndex, 0)` to refresh only that prompt's turns.
...
- **Realtime `session.message` events**: upsert message by `(sessionId, promptIndex, turnIndex)`, advance cursor if incoming `(promptIndex, turnIndex)` is greater than current, method is not `prompt_done`, and agent text/thought turns are `done: true`.
- **`session.read` active pull**: pull delta from checkpoint, replace messages after affected prompt boundary.
- **`prompt_done` completeness refresh**: `prompt_done` is still written into the message stream, but only acts as the prompt-ending event and turn watermark; when a gap is detected, read only that prompt.
...
interface RegistrySessionMessage {
  ...
  done?: boolean;                     // turn envelope completion for agent text/thought
}
```
...
- `agent_message_chunk` / `agent_thought_chunk` seal rule: when the next different turn or `prompt_done` arrives, server republishes the same `(promptIndex, turnIndex)` with the complete text and `done: true` on the envelope.

### 10.3 Persistence

- On `prompt_request` arrival: create `sessionPromptState`, record `SessionPromptRecord` (status=started).
- On `prompt_done` arrival: encode in-memory turns as JSON, save final `SessionPromptRecord` (with `stopReason`, `TurnsJSON`, `RecordedAt`), remove prompt state from memory.
- On restart: load latest prompt's `TurnsJSON` from SQLite to restore `sessionPromptState`.
