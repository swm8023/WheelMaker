...
> **`done` 是 turn envelope 状态**：仅用于 `agent_message_chunk` / `agent_thought_chunk`，表示该 turn 已封口；不是 prompt 完成标记。`content.param.text` 始终是当前 turn 的完整文本快照。
...
4. 通过 `(promptIndex, turnIndex)` 游标去重；`prompt_done` 不推进读取游标。
5. 将消息 upsert 到 `chatMessageStoreRef`。
6. 若 `sessionId === chatSelectedIdRef.current`（当前查看的会话），更新 `chatMessages` 触发 UI 刷新。
7. 收到 `prompt_done` 后，如当前 prompt 有未 `done` 的 text/thought turn，或本地最大 turn 小于 `prompt_done.turnIndex - 1`，调用 `session.read(sessionId, promptIndex, 0)` 仅补读当前 prompt 的 turns。
...
- **实时 `session.message` 事件**：按 `messageId` upsert 消息，若 incoming 的 `(promptIndex, turnIndex)` 大于当前游标、method 不是 `prompt_done`，且 agent text/thought turn 已 `done: true`，推进游标。
- **`session.read` 主动拉取**：从检查点拉增量，替换受影响 prompt 边界之后的消息。
- **`prompt_done` 完整性补读**：`prompt_done` 正常写入消息流，但仅作为 prompt 结束事件和 turn watermark；发现当前 prompt 有缺口时只读取该 prompt。
...
- `agent_message_chunk` / `agent_thought_chunk` 封口规则：当下一个不同 turn 或 `prompt_done` 到达时，服务端重发同一 `(promptIndex, turnIndex)` 的完整文本，并在 envelope 上设置 `done: true`。

### 10.3 持久化

- `prompt_request` 到达时：创建 `sessionPromptState`，记录 `SessionPromptRecord`（status=started）。
- `prompt_done` 到达时：将内存中的 turns 编码为 JSON，保存最终 `SessionPromptRecord`（含 `stopReason`、`TurnsJSON`、`RecordedAt`），从内存中移除 prompt state。
- 重启后从 SQLite 加载最新 prompt 的 `TurnsJSON` 恢复 `sessionPromptState`。
