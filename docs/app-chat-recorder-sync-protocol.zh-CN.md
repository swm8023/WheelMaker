...
> **前端处理**：`session.read` 返回的 `messages[]` 在前端经 `decodeSessionMessageFromEventPayload` 解码后，按 `syncIndex`（= `promptIndex`）分组为 `prompts` 数组。最终返回 `{session, prompts, messages}` 供 UI 使用。
...
4. 通过 `(syncIndex, syncSubIndex)` 游标去重（仅处理更新的消息）。
...
- **实时 `session.message` 事件**：按 `messageId` upsert 消息，若 incoming 的 `(syncIndex, syncSubIndex)` 大于当前游标，推进游标。
