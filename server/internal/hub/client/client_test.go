...
// Legacy black-box tests were replaced by smaller runtime- and persistence-focused tests.
/*
...
func TestHandleMessage_Use_UnknownBackend(t *testing.T) {
...
func TestJSONStore_SaveWritesNewKeys(t *testing.T) {
	...
	if _, ok := raw["activeBackend"]; ok {
		t.Error("saved file should not have flat 'activeBackend' key (must be nested under 'projects')")
	}
}
...
// TestSwitchBackend_PreservesIncomingSessionIDOnCleanSwitch verifies that a plain
...
func TestSwitchBackend_PreservesIncomingSessionIDOnCleanSwitch(t *testing.T) {
