...
func TestSendToolCall_YoloReadEditBypassCompact(t *testing.T) {
	kinds := []string{"read", "edit"}
	for _, kind := range kinds {
		t.Run(kind, func(t *testing.T) {
			...
			if err == nil {
				t.Fatalf("sendToolCall(%s) should fail without app credentials", kind)
			}

			if st := f.toolCompact["chat-1"]; st != nil && len(st.order) > 0 {
				t.Fatalf("%s should bypass compact stream, got order=%v", kind, st.order)
			}
			if cards := f.toolCards["chat-1"]; cards == nil || cards["call-1"] == nil {
				t.Fatalf("%s should create/update toolCards state", kind)
			}
		})
	}
}

func TestSendToolCall_ReadStartsNewCardInsteadOfUpdating(t *testing.T) {
	f := newTransport(Config{})
	f.toolCards["chat-1"] = map[string]*toolCardState{
		"call-1": {
			messageID: "msg-old",
			update: ToolCallUpdate{
				ToolCallID: "call-1",
	...
	st := f.toolCards["chat-1"]["call-1"]
	if st == nil {
		t.Fatal("tool state missing after read update")
	}
	if st.messageID != "" {
		t.Fatalf("read update should force new card (empty messageID), got %q", st.messageID)
	}
	if st.update.Status != "completed" {
		t.Fatalf("status=%q, want completed", st.update.Status)
	}
}

func TestSendToolCall_BashKeepsExistingMessageID(t *testing.T) {
	f := newTransport(Config{})
	f.toolCards["chat-1"] = map[string]*toolCardState{
		"call-1": {
			messageID: "msg-old",
	...
	if err == nil {
		t.Fatal("sendToolCall(bash) should fail without app credentials")
	}

	st := f.toolCards["chat-1"]["call-1"]
	if st == nil {
		t.Fatal("tool state missing after bash update")
	}
	if st.messageID != "msg-old" {
		t.Fatalf("bash update should keep existing messageID for in-place update path, got %q", st.messageID)
	}
}
