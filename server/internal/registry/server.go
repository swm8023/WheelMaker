...
func (s *Server) handleWS(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case "chat.send",
			"session.list", "session.read", "session.new", "session.resume.list", "session.resume.import", "session.reload", "session.send", "session.markRead", "session.setConfig", "session.delete", "session.token.providers", "session.token.deepseek.stats",
