...
// Agent-level info (from initialize) and the most recent session-level info
// (modes, models, available commands, config options) are stored flat here;
// there is no per-session nesting because only the current session's data
// is meaningful to persist.
type AgentState struct {
	...
	// Session-level metadata refreshed after each session/new or session/update.
...
// It stores the active adapter name and per-adapter metadata.
