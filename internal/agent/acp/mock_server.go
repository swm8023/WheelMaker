...
// NewInMemoryMock creates an ACP connection backed by a built-in mock agent.
func NewInMemoryMock() *Conn {
	return NewInMemory(runInMemoryMockServer)
}
...
func (s *inMemoryMockServer) handleRequest(id int64, method string, params json.RawMessage) {
	...
	case "initialize":
		s.respond(id, map[string]any{
			...
			"agentInfo": map[string]any{
				"name":    "wheelmaker-mock",
				"title":   "WheelMaker Mock Agent",
				"version": "0.1.0",
			},
			"authMethods": []any{},
		})
	...
	case "session/set_config_option":
		...
		s.respond(id, s.buildConfigOptions(state))
...
func (s *inMemoryMockServer) handlePrompt(id int64, params json.RawMessage) {
	...
	case "1":
		s.sendUpdate(p.SessionID, map[string]any{
			...
			"content":       map[string]any{"type": "text", "text": "mock text chunk"},
		})
		s.sendUpdate(p.SessionID, map[string]any{
			"sessionUpdate": "agent_thought_chunk",
			"content":       map[string]any{"type": "text", "text": "mock thought chunk"},
		})
		s.sendUpdate(p.SessionID, map[string]any{
			"sessionUpdate": "plan",
			"entries": []map[string]any{
				{"content": "step1", "priority": "high", "status": "pending"},
				{"content": "step2", "priority": "medium", "status": "in_progress"},
			},
		})
		s.sendUpdate(p.SessionID, map[string]any{
	...
	case "2":
		readRaw, readErr := s.callbackRequest("fs/read_text_file", map[string]any{
			"sessionId": p.SessionID,
			"path":      "/mock/file.txt",
		})
		readContent := "fs-read-error"
		if readErr == nil {
			var rr FSReadTextFileResult
			_ = json.Unmarshal(readRaw.Result, &rr)
			if rr.Content != "" {
				readContent = rr.Content
			}
		}
		_, _ = s.callbackRequest("fs/write_text_file", map[string]any{
			"sessionId": p.SessionID,
			"path":      "/mock/written.txt",
			"content":   readContent,
		})
		s.sendUpdate(p.SessionID, map[string]any{
			"sessionUpdate": "agent_message_chunk",
			"content":       map[string]any{"type": "text", "text": "fs:" + readContent},
		})
		s.respond(id, map[string]any{"stopReason": "end_turn"})

	case "3":
		createRaw, err := s.callbackRequest("terminal/create", map[string]any{
			"sessionId": p.SessionID,
			"command":   "echo",
		...
		var cr TerminalCreateResult
		_ = json.Unmarshal(createRaw.Result, &cr)
		_, _ = s.callbackRequest("terminal/output", map[string]any{
			"sessionId":  p.SessionID,
			"terminalId": cr.TerminalID,
		})
		_, _ = s.callbackRequest("terminal/wait_for_exit", map[string]any{
			"sessionId":  p.SessionID,
			"terminalId": cr.TerminalID,
		})
		_, _ = s.callbackRequest("terminal/release", map[string]any{
			"sessionId":  p.SessionID,
			"terminalId": cr.TerminalID,
		})
		s.sendUpdate(p.SessionID, map[string]any{
			"sessionUpdate": "agent_message_chunk",
			"content":       map[string]any{"type": "text", "text": "terminal:ok"},
		})
		s.respond(id, map[string]any{"stopReason": "end_turn"})
	...
	case "4":
		...
		permRaw, err := s.callbackRequest("session/request_permission", map[string]any{
			"sessionId": p.SessionID,
...
func (s *inMemoryMockServer) callbackRequest(method string, params any) (rawMessage, error) {
	id := s.nextID.Add(1)
	...
	case resp := <-respCh:
		if resp.Error != nil {
			return rawMessage{}, fmt.Errorf("rpc error %d: %s", resp.Error.Code, resp.Error.Message)
		}
		return resp, nil
	case <-time.After(3 * time.Second):
		...
		delete(s.pending, id)
		s.pendMu.Unlock()
		return rawMessage{}, fmt.Errorf("timeout waiting callback %s", method)
	}
}

func (s *inMemoryMockServer) routeResponse(id int64, resp rawMessage) {
	s.pendMu.Lock()
	ch, ok := s.pending[id]
	if ok {
		delete(s.pending, id)
